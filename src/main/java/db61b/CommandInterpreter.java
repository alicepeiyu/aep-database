...
class CommandInterpreter {
    ...
    Table selectClause() {
        ...
        if (_input.nextIs("where")) {
            if (table2 == null) {
                ArrayList condList = conditionClause(table1);
                Table result1 = table1.select(columns, condList);
                return result1;
            } else {
                ArrayList condList = conditionClause(table1, table2);
                Table result2 = table1.select(table2, columns, condList);
                return result2;
            }
        } else {
            if (table2 == null) {
                return table1.select(columns, new ArrayList<>());
            } else {
                return table1.select(table2, columns, new ArrayList<>());
            }
        }
    }
    ...
    Condition condition(Table... tables) {
        String col1Str = columnName();
        Column col1 = new Column(col1Str, tables);
        String relation = _input.next(Tokenizer.RELATION);

        if (_input.nextIs(Tokenizer.IDENTIFIER)) {
            String col2Str = columnName();
            Column col2 = new Column(col2Str, tables);
            Condition cond = new Condition(col1, relation, col2);
            return cond;
        } else {
            String val2 = literal();
            Condition cond = new Condition(col1, relation, val2);
            return cond;
        }
    }
